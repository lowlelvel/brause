...
def parse_ip_port(line):
    ...
    Accepts:
      - 1.1.1.1
      - 1.1.1.1:80
    Returns (ip, port). Defaults to port 443 when omitted.
    """
    line = line.strip()
    if ':' in line:
...
def rename_vmess_address(proxy, new_address, new_port):
    ...
    try:
        ...
        proxy_json['port'] = str(new_port)
        proxy_json['ps'] = f"{new_address}:{new_port}"  # Set remarks to new address:port
        proxy_counter += 1
...
def rename_vless_address(proxy, new_address, new_port):
    global proxy_counter
    try:
        parts = proxy.split('@', 1)
        userinfo = parts[0]
        ...
        # Preserve query/fragment part if present
        suffix = ''
        cut_pos = len(hostinfo)
        for sep in ('?', '#'):
            pos = hostinfo.find(sep)
            if pos != -1 and pos < cut_pos:
                cut_pos = pos
        if cut_pos < len(hostinfo):
            suffix = hostinfo[cut_pos:]
            hostinfo = hostinfo[:cut_pos]
...
def rename_trojan_address(proxy, new_address, new_port):
    global proxy_counter
    try:
        parts = proxy.split('@', 1)
        userinfo = parts[0]
...
def process_proxies(input_file, ips_file, output_file):
    ...
    # Read the list of IP addresses / IP:PORT entries, skipping lines that start with "//"
        ...
        for proxy in proxies:
            for ip, port in ips:
                if proxy.startswith('vmess://'):
                    renamed_proxy = rename_vmess_address(proxy, ip, port)
                elif proxy.startswith('vless://'):
                    renamed_proxy = rename_vless_address(proxy, ip, port)
                elif proxy.startswith('trojan://'):
                    renamed_proxy = rename_trojan_address(proxy, ip, port)
                else:
